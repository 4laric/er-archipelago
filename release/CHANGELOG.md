...
- **Client:** Optional for seed compatibility; update for the Margit region-lock fix (existing runs too), Region Sync re-announcing standing opens, and lighter enemy-scaling sweeps. No reroll.
- **APWorld:** Host-only, for newly generated rooms after this version ships.
- **YAML:** **New YAML optional. Existing YAMLs remain valid.** A YAML that leaves out `ending_condition` and `goal_region_unlock_policy` now rolls the new default goal (below); set them explicitly to keep the old one.
- **Existing seed/save:** Compatible; no regeneration or save migration required.
- **Profile/assets:** No action; no map or asset changes.

### Compatibility -- read this first

**The v0.6.1 client plays every run generated by an older apworld. No reroll, no save migration, no YAML change is needed to update it.**
...
- **Margit region lock (existing and new seeds):** distinguish the exact arena play-region ID
  ...
  clients #693)
- **Region Sync re-announces standing opens (client):** the broadcast only fired on a live Lock
  receipt, so a start region, or one opened while a partner was offline, never reached them and
  they were region-kicked from somewhere you were standing. Open regions are now re-sent every
  120s and receivers skip ones already open. (clients #695)
- **Enemy-scaling sweep backs off when nothing changes (client):** once a region has converged
...
This is the v0.6.0.12 window, renamed. v0.6.0.12 was opened on 2026-09-14 and never tagged, so
...
`release/CHANNELS.tsv` promotes `stable` to v0.6.0.11 in this same window.
...
- **Castle Ensis region pass (#1554):** Corrected 7 checks around Castle Ensis after a playtester report, adjudicated per check rather than per tile. The Scadutree Fragment at Highroad Cross and a Ghost Glovewort move to Scadu Altus; Moonrithyll's Knight Sword, the Loyal Knight's Cookbook, Starlight Shards, the Carian Sorcery Sword and a Grave Glovewort [8] move from Gravesite to Ensis. Six further reported checks were measured and did NOT move. Region locks and sweep ownership follow the corrected homes in new rooms; AP IDs are unchanged.
...
- **A missable check set from two maps was labelled "questline", and the oracle never checked the
  ...
  `questline` -- including the multi-site class, whose members are missable because the flag is set
  from several maps (flag 60510 is set from both `m10_00` and `m11_00`), so the item may not be
  obtainable at every site and the visit order decides which one still has it. No quest is
  involved, and this file's own doctrine is that the reason string is a claim about the MECHANISM
  (the same reason `gesture_award` and `questline_item` exist). 29 checks now carry a truthful
  label instead: 28 `multisite` and 1 `cross_region_prereq` (the Belurat message gated on Messmer,
  a prerequisite in ANOTHER region). **WHICH checks are missable is unchanged** -- same 291 rows,
  same ap_ids, same protection -- so no seed, pool or contract effect; only the reason moves.
  (2) `tools/matt_oracle.py --report` gained the missable join it never had: our
  `MISSABLE_LOCATIONS` flags against his Event-scope slots tagged `missable`. First run: 291 rows
  / 210 distinct flags ours, 168 of his missable flags joinable against `data.LOCATIONS`, 100
  agree, 68 his-only, 110 ours-only. **Report-only and never a gate** -- he tags a SLOT ("do not
  randomize this into oblivion") and we tag a CHECK ("must not host REQUIRED progression"), so a
  one-sided flag is a worklist entry, not a defect. The tag name is filter vocabulary only; no row,
  text, area name or tag string of his is committed, and the new test fixture is synthetic like
  the two before it. Roadmap item 5 in `docs/MATT-ORACLE-ROADMAP.md`.

- **The 218 region disagreements the matt oracle reports now have a review queue instead of a
  paragraph.** The oracle's third class is region assignment, and it has always been report-only
  because the two tables partition the world differently — but "218 rows disagree" written in a
  roadmap is not something two people can work through. `matt_oracle.py --region-queue` now writes
  `greenfield/evidence/oracle-region-queue.tsv`: one row per disagreeing check, keyed on OUR flag
  and ap_id, with `status` (open / confirmed-ours / moved), `reviewer` and `note`, carried across
  a refresh so a verdict is never lost to a re-run. The two DLC-membership flags (520800, 530950)
  are queued unconditionally, because a model difference that happens to cancel out is not
  evidence the row is right. The check browser gains an **Oracle region review** facet and the
  player review notebook a **queue view**, both putting OUR OWN evidence beside each row — the
  assigned region, the derivation step that assigned it, the map tile, the nearest grace and the
  ...
  🛑 **Licence boundary, unchanged and now asserted in the tests.** The queue file records only
  THAT a second source's partition disagrees for a flag. His area names, `Text` and tags are not
  in it, are not in either page, and never reach disk: the area label is an equivalence key inside
  the process and the mapping it produces is built from OUR region names. Both browsers read the
  committed tsv only, so **CI never needs his checkout** and the generators diff gate stays
  deterministic. 🛑 **This PR queues; it does not fix.** Ruled rows move through the normal
...
- **The v0.6 evidence bundle is regenerated by `tools/regen_all.py`, not by hand.**
  `tools/build_v060_current_evidence.py` was outside the single regen entrypoint even though
  every claim and evidence row embeds the sha256 of its source module, so any edit to `data.py`
  and friends re-stales all ~20k rows and only
  `test_gf_v060_current_evidence.py::test_checked_in_bundle_validates_and_is_byte_deterministic`
  ever said so — the #699 hole wearing a third hat. #1490's merge shipped exactly that, and the
  rebuild plus its two dependants had to be applied by hand (2cd6e8ea, f3141dd1). The builder now
  runs FIRST in the `tables` phase (~1s, no phase flag needed), followed by
  `check_evidence_baseline.py --refresh`; `build_evidence_browser.py`, which embeds the bundle
  digest, already ran in `pages` after it. `--refresh` is a new, deliberately narrow flag: it
  re-baselines only when `content_hash` is the sole difference, so a mechanical rebuild needs no
  human while a real move in the audited census still reports DRIFT and stops the entrypoint —
  an unconditional `--update` in a regen step would have quietly deleted that review gate.
  **Docs and tooling only: no seed, pool or contract effect**, and the checked-in bytes are
  unchanged (regen_all is byte-stable on a second run).

- **The v0.6.0.6 Elden Ring 2.7.1.0 bullet was listed twice.** #1491's window move left a bare
  copy of the bullet immediately above the annotated "moved here" version. The bare copy is gone;
  no other bullet in the section is duplicated.
...
- **YAML:** **No new YAML required. Existing YAMLs remain valid.**
- **Existing seed/save:** Compatible — a fixpack never strands a running seed.
- **Profile/assets:** No action — the MapForGoblins build and preset are unchanged from v0.6.0.3.
Window opened AT THE TAG of v0.6.0.3 with ZERO commits past it.

`CONTRACT_HASH` is `613fb438`, read by loading contract.py: unmoved since v0.6.0.3, so a
v0.6.0.3 client and a v0.6.0.4 apworld pair without a mismatch in either direction. Versions are
V.R.M.F: a client on the 0.6.0 line from v0.6.0.3 on plays every seed the line generates.

The version moved, so the client half moved with it: clients PR #657 "Stamp the paired client
for the v0.6.0.4 window" moves the three client version sites, and the gitlink rides in this same
commit.

`release/CHANNELS.tsv` promotes `stable` to v0.6.0.3 in this same commit.
...
Window opened 1 commit(s) PAST the v0.5.3 tag.

`CONTRACT_HASH` remains `13db0b3a`, verified by loading `contract.py`; the slot-data shape is
unchanged, while the exact-version handshake advances to 0.5.4.

Client half: clients#477. Its merged commit is pinned by the gitlink in this same change because
`contract_gen.rs` embeds the exact APWorld version even when the contract hash does not move.

`release/CHANNELS.tsv` promotes `stable` to v0.5.3 in this same commit.
...
- **The pinned client can run audited v0.4.13 seeds after a game/client update.** Compatibility is
  limited to the exact historical `dc0dc687` contract; every other version/contract mismatch keeps
  the existing warning and refusal behavior. This lets players whose game updated mid-seed finish
  without weakening the general paired-version boundary.
