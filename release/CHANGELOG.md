...
- **A boss no longer sweeps checks in a region it is not fought in.** Reported by **NovahDango** and **Lilith** (Discord, 2026-08-26) and ruled the same day: *"there shouldn't be any cross-region boss sweeps in general."* NovahDango's case: five `Abyssal ::` rows around the Church Ruins and the Abyssal Woods -- Clarifying Boluses, Frenzyflame Perfume Bottle, Scadutree Fragment, Shadow Realm Rune, Swollen Grape -- read "also granted by Jori, Elder Inquisitor", and Jori is fought in Midra's Manse territory, not in the Abyssal Woods. Our own tables agree with him: `boss_area_regions.tsv` puts Jori's arena in play_region 40020 and `REGION_PLAY_IDS` gives 40020 to Scadu Altus. 🛑 THIS IS NOT A LABEL PROBLEM. A region lock buys a region; a trigger fought in A that pays members presenting in B shows a player who bought A a list of checks they do not own, and walking to one puts them in front of the kick guard for a play_region their seed never opened -- #330's shape, and the Hippo rule (#885) is this same sentence said once about one boss. The mechanism is the field pass: it groups a boss with its NEIGHBOURHOOD (Chebyshev <= 2 tiles) and takes the MEMBERS' region as the sweep's, so a boss standing just over a border legitimately reaches the other side. `SWEEP_ARENA_REGION` has recorded where each boss is FOUGHT since #445 and gen_data already PRINTED the split -- it simply never cut it, and the invariant had been written down only for own-drop admission ("a sweep may only grant checks in ITS OWN region"). Whole-corpus census: **4 triggers / 22 cross-region member links** on `9dae59b4`, rising to 26 once the region moves land. **Three of the four were the ARENA attribution being stale against a ruling the CHECKS already follow**, not cross-region grants, and are curated in `_ARENA_REGION_CURATED` (the existing Margit precedent) rather than cut: `34100800` Divine Tower of Limgrave, whose whole m34_10 bucket was ruled into Stormveil (#202; `region_overrides.tsv` carries all seven 34107xxx rows), and `11050800`/`11050850`, fought inside the deliberate Ashen Capital finale sub-carve of Leyndell's kick bucket 11050. Only Jori's was real. The containment pass cuts every remaining cross-region member, re-homes each to the smallest trigger whose sweep AND arena region are the member's own, and drops a trigger left holding nothing (the `SWEEP_UNSPAWNED` shape): **Jori's 9 all re-home inside Abyssal to trigger `28000800`, no member loses its last host, no region's pool empties** (the Astel clawback concern), and Jori is dropped -- 211 → 210 triggers, member links unchanged at 4100. A narrowing was DIFFED rather than trusted to a green suite: the whole `DUNGEON_SWEEPS` corpus is compared before and after, and the only member-set change anywhere is that one move. Lilith's case is the OTHER shape and got the other fix: `Belurat :: Message from Leda - near Scaduview Cross, also granted by Divine Beast Dancing Lion` -- *"this one is in Shadow Keep, not in Belurat"* -- was filed Belurat only because the Lion swept it, so the REGION was corrected (`FLAG_REGION_OVERRIDE[580600]`, in the region change alongside this one) and the check now rides Dryleaf Dane in its own region instead of losing its link. Some checks carry no "also granted by" line at all, which NovahDango also noticed: that is the non-swept remainder and is unchanged. Gated by `assert not _sweep_arena_split` in `gen_data.py` -- the pass's own witness, which cannot pass while any split survives -- plus `test_gf_sweep_containment.py` (8 tests): the whole-corpus invariant, `SWEEP_REGION` equalling `SWEEP_ARENA_REGION`, Jori granting no Abyssal check, the cut members still swept by an Abyssal host, Leda no longer Belurat and no longer the Lion's, and a ceiling on the 26 UNAUDITED triggers -- an absent arena row is UNMEASURED, not clean, and this invariant is blind to every one of them, so their number may not grow in silence. (#1057)
