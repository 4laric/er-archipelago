...
- **`maximum_enemy_difficulty: auto` is the base game's top unless the Scadutree Blessing is in
  play everywhere.** Every ladder rung above 3.70x (vanilla Haligtree) is the DLC's own enemy
  ladder, tuned for a player carrying a blessing. A 13-region default run met Caelid at close to
  Haligtree strength. `auto` now resolves to 47 (3.70x) flat, and climbs only when
  `scadutree_blessing_scope` is `anywhere` AND the DLC is on so fragments can enter the pool; the
  fragment injection is sized from the same resolved cap. The climb itself was also recalibrated
  against the live 28-region total: 5 regions 3.70x, 10 regions 5.48x, 15 regions 6.69x, 20
  regions 7.05x, a whole map unchanged at 7.42x. Explicit percents are untouched. A seed rolled
  from the shipped yaml (DLC off) now has a cap below 100 and needs a client that understands
  capping, which every current client does.
