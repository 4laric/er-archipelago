...
- **The check browser no longer promises you a sweep it cannot know about (#936).** colombius asked why `Mountaintops of the Giants :: Golden Seed - near Foot of the Forge, by two snow trolls, also granted by Fire Giant (m60_52_52) [f1052537800]` was not handed over by the Fire Giant. Nothing was broken: the check is tagged `Seedtree`, which is on the default `progression_surface`, so the per-seed surface cut correctly took it back out of that sweep — that is where the seed hid its Locks. The NAME said otherwise, because location names ride the **static** AP datapackage and cannot vary per seed. The in-game client has filtered that clause against the seed's real membership since v0.5.0 (slot_data `dungeonSweepFlags`), but the **check browser has no seed at all** — it is built once from the corpus — and it was showing the clause inside the name as if it were a promise. It now splits the clause out and renders it as a `Sweep: eligible — Fire Giant (m60_52_52)` row that says in as many words that `dungeon_sweep` and `progression_surface` decide. 3223 of the 3943 names on the page are affected; the boss stays in the search haystack, because "which checks does Fire Giant sweep" is exactly what the page is asked, and the `[fNNNN]` tail stays on the name, because that is the check's identity in a bug report. **The datapackage names themselves are unchanged in this release** — rewording those fixes the remaining surfaces (spoiler logs, `!hint`, text clients) but renames 82% of the corpus, so it is filed for a ruling rather than taken silently. No behaviour, no contract, no client change.
