...
| `me3/` | The runtime client folder. Holds `eldenring_archipelago.dll` (the MIT client), the `ap.me3` profile, `apconfig.json`, the local AP-flower installer/payload, and two **required** data tables (`check_lots_table.json`, `shoplineup_flags.json`). Keep these together. Run `me3\install-ap-flower.ps1` once before launching; it creates `ap-package/` from your installed game. |
