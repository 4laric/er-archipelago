...
def main():
    ...
    for f in sorted(found):
        ...
        if len(bs) == 1:
            a = arenas[bs[0]]
            emitted.append({
                "kind": "item", "key": f, "map_id": a["boss_map"], "x": a["local_x"],
                "y": a["local_y"], "z": a["local_z"], "name": "",
                "via": "boss_arena", "arena_defeat_flag": bs[0],
    ...
    # SCHEMA-COMPATIBLE with item_grace_coords.tsv on purpose: the first seven columns are exactly
    # `kind key map_id x y z name`, so build_nearest_grace.parse_coords reads this file natively
    # with no special case, and the derived-only columns ride along after it where a positional
    # reader ignores them. Compatible to READ; still a separate FILE, so nothing merges by accident.
    hdr = ["kind", "key", "map_id", "x", "y", "z", "name", "via", "arena_defeat_flag", "evidence"]
