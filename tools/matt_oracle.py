...
factually. `--report` also prints two count-only comparisons that gate nothing:
  ...
  C. BOSS TAXONOMY -- our per-class boss histogram (`boss_taxonomy.BOSS_CLASS_COUNTS`, derived
  ...
  D. REACHABILITY COVERAGE -- one line: how many regions and grace-warp groups our logic graph can
...
# ---------------------------------------------------------------------------
# C. BOSS TAXONOMY -- report-only histogram (docs/MATT-ORACLE-ROADMAP.md item 7).
#
...
def report_taxonomy_and_reach(repo, souls_dir, rows):
    """Print C (boss-class histogram) and D (reachability coverage). Returns the JSON payload."""
    ...
    print("== C. BOSS TAXONOMY (boss_taxonomy.BOSS_CLASS_COUNTS) -- REPORT ONLY ==")
    ...
    print("== D. REACHABILITY COVERAGE -- REPORT ONLY ==")
...
def main(argv=None):
    ...
    # --- C + D: report-only, and only under --report ---
