...
# ---------------------------------------------------------------------------
# J. CORROBORATED HOSTS -- the generated table features/evidence_progression_hosts promotes from.
#
# A check is CORROBORATED when its flag joins one of his Event-scope slots (so a second source
# agrees on what the flag awards; section A is 99.8% on that join) AND no review queue currently
# disputes it (a region or missable row whose status is anything but `confirmed-ours`). That is
# the "his table agrees" definition, and on its own it is NOT a hosting rule: the feature
# intersects it with sweep membership and subtracts the missable set before a single advancement
# item may land on it. The table is bare ap-ids of OURS, produced here because only this tool can
# see his checkout; it is refreshed with the pin, exactly like the allowlists above.
# ---------------------------------------------------------------------------
DEFAULT_HOSTS_TABLE = os.path.join(REPO, "greenfield", "eldenring", "tables",
                                   "oracle_corroborated_hosts.py")
DISPUTED_STATUSES = frozenset({"open", "moved", "missable"})


def check_corroborated(rows, by_flag, region_queue, missable_queue):
    """Sorted ap-ids of OUR rows that join an Event slot and sit in no disputing queue row.

    `region_queue` / `missable_queue` are the committed queues as read_queue returns them:
    {(flag, ap_id): row}. A row disputes when its status is in DISPUTED_STATUSES; a
    `confirmed-ours` / `confirmed-not-missable` ruling does not.
    """
    event = {r["flag"] for r in rows if r["stype"] == SCOPE_EVENT and r["flag"]}
    disputed = set()
    for queue in (region_queue, missable_queue):
        for (_flag, ap_id), row in queue.items():
            if (row.get("status") or "open") in DISPUTED_STATUSES:
                disputed.add(ap_id)
    out = []
    for flag, entries in by_flag.items():
        if flag not in event:
            continue
        for _region, _name, ap_id in entries:
            if ap_id not in disputed:
                out.append(ap_id)
    return sorted(out)


HOSTS_TABLE_HEADER = '''"""AUTO-GENERATED by tools/matt_oracle.py --hosts-table -- DO NOT EDIT.

Checks whose event flag a second, independently curated table of the same game also carries, and
that no oracle review queue currently disputes. Refreshed by hand against a local
thefifthmatt/SoulsRandomizers checkout at the pin in .github/workflows/matt-oracle.yaml, like the
allowlists in that tool; it is NOT a tools/regen_all.py step because it needs that checkout.

NOT a hosting rule by itself: features/evidence_progression_hosts.oracle_sweep_aps intersects it
with sweep membership and subtracts the missable set. Every value is one of OUR ap-ids; nothing of
the second source is here (AGENTS.md "MATT ORACLE").
"""
'''


def write_hosts_table(path, ap_ids, pin):
    lines = [HOSTS_TABLE_HEADER, "# second source at %s; %d checks" % (pin, len(ap_ids)),
             "ORACLE_CORROBORATED_APS = frozenset(("]
    for start in range(0, len(ap_ids), 12):
        lines.append("    " + ", ".join(map(str, ap_ids[start:start + 12])) + ",")
    lines.append("))")
    with open(path, "w", encoding="utf-8", newline="\n") as fh:
        fh.write("\n".join(lines) + "\n")
...
def main(argv=None):
    ...
    ap.add_argument(
        "--hosts-table", dest="hosts_table", nargs="?", const=DEFAULT_HOSTS_TABLE, default=None,
        metavar="PATH",
        help="write the corroborated-hosts table (default: %s) from the join and the COMMITTED "
             "queues" % os.path.relpath(DEFAULT_HOSTS_TABLE, REPO),
    )
    ap.add_argument("--pin", default=None,
                    help="the second source's commit, recorded in the hosts table header")
    ...
    # --- J (a generated table, on request) ---
    corroborated = check_corroborated(
        rows, by_flag,
        read_queue(args.region_queue or DEFAULT_REGION_QUEUE),
        read_queue(args.missable_queue or DEFAULT_MISSABLE_QUEUE))
    print("== J. CORROBORATED HOSTS (the generated promotion table) ==")
    print("%d of %d rows join an Event slot of his and sit in no disputing queue row"
          % (len(corroborated), n_ours))
    if args.hosts_table:
        pin = args.pin
        if pin is None:
            try:
                import subprocess
                pin = subprocess.check_output(["git", "-C", d, "rev-parse", "HEAD"],
                                              text=True).strip()
            except Exception:
                pin = "unknown"
        write_hosts_table(args.hosts_table, corroborated, pin)
        print("  wrote %s (%d ap-ids)" % (args.hosts_table, len(corroborated)))
    print()
    ...
    if args.json_out:
        payload = {
            ...
            "corroborated_hosts": len(corroborated),
            "uncovered": {
                "our_rows": n_ours,
                "joined": unc_joined,
                "uncovered": len(unc),
