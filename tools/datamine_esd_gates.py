...
def main():
    ...
    if args.probe:
        ...
        for (t, gf, gs, a, b) in all_shops[:40]:
            g = f"EventFlag({gf})=={gs}" if gf != -1 else "UNGATED"
            print(f"  shop  talk {t}: OpenRegularShop({a},{b})  gate={g}")
        if len(all_shops) > 40:
            print(f"  ... and {len(all_shops) - 40} more shops")
        print(f"# GIFTS (AwardItemLot; gate_flag with sense 0 is the 'not yet received' check flag):")
        for (t, gf, gs, lot) in all_gifts[:40]:
            g = f"EventFlag({gf})=={gs}" if gf != -1 else "UNGATED"
            print(f"  gift  talk {t}: AwardItemLot({lot})  gate={g}")
        if len(all_gifts) > 40:
            print(f"  ... and {len(all_gifts) - 40} more gifts")
        return 0
    ...
    # conversation. It almost always sits behind `if EventFlag(received)==0:` and is followed by
    # `SetEventFlag(received, 1)`, so the gate_flag with gate_sense==0 IS the "not yet received"
    # acquisition flag -- i.e. the AP check flag. talk_id -> the NPC's map (join as in merchant_shops)
    # -> region. Emitted for cross-checking against the existing location set: a gift whose flag is not
    # already a check is a candidate to place (region from the talk's map), and one that is already a
    # check gets its region confirmed.
    ...
    with open(gifts_out, "w", encoding="utf-8", newline="\n") as f:
        ...
        f.write("# received' acquisition flag (the AP check flag); item_lot is the ItemLotParam handed\n")
        f.write("# over; talk_id -> NPC map -> region (join as in datamine_merchant_shops).\n")
        f.write("talk_id\tgate_flag\tgate_sense\titem_lot\n")
        for (t, gf, gs, lot) in all_gifts:
            f.write(f"{t}\t{gf}\t{gs}\t{lot}\n")
    _received = sum(1 for (_t, gf, gs, _l) in all_gifts if gf != -1 and gs == 0)
    print(f"esd_gifts: {len(all_gifts)} gift(s), {_received} with a received-flag gate -> {gifts_out}")
    return 0
