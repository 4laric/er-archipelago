...
def _er_yaml(name, natural, confine=None):
    ...
    if confine is not None:
        # The three sharing knobs are hidden since 2026-09-18 and no longer carry a template line;
        # `progression_sharing` does. A hidden option is still accepted in a yaml, so the matrix sets
        # the share by writing the line straight under progression_sharing.
        if not re.search(r"^\s*progression_sharing:.*$", s, flags=re.M):
            sys.exit("FAIL: release/EldenRing.yaml no longer carries a progression_sharing line, so "
                     "this test silently stopped setting the share it claims to test.")
        s = re.sub(r"^(\s*)progression_sharing:.*$",
                   r"\g<1>progression_sharing: balanced\n\g<1>confine_foreign_progression: %d" % confine,
...
def self_test():
    ...
    # The shipped default is `progression_sharing: balanced` == confine 100. Exercise that exact
    # endpoint so this guard cannot regress to a change-detector that rejects a legal default.
    rendered = _er_yaml("SelfTest", False, 100)
    if "confine_foreign_progression: 100" not in rendered:
