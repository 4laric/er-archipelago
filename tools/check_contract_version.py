...
def main(argv):
    ...
    if errs:
        ...
        return 1
    # FIXPACK NOTE (V.R.M.F, Alaric 2026-09-06). A fixpack promises that a client on this V.R.M can
    # be swapped onto any seed generated by any apworld on the line. The contract hash MAY move on a
    # fixpack -- provided the new client bridges the previous hash (er-logic client_features
    # is_legacy_contract_compatible). This gate cannot see the client, so it names the obligation
    # rather than failing: the reviewer of the fixpack owns the bridge.
    parts = version.split(".")
    if len(parts) == 4 and parts[3] != "0":
        line = ".".join(parts[:3])
        prev = [v for v in order if v == line or v.startswith(line + ".")]
        prev = [v for v in prev if v != version]
        moved = sorted({rows[v][0] for v in prev if rows[v][0] != chash})
        if moved:
            print("%sNOTE%s v%s is a fixpack on the %s line and its contract (%s) differs from an "
                  "earlier release on that line (%s). That is allowed ONLY if the client shipped "
                  "with it bridges the older contract (is_legacy_contract_compatible must list the "
                  "pair); a %s.x client must still play every %s seed."
                  % (GRN, OFF, version, line, chash, ", ".join(moved), line, line))
        else:
            print("  fixpack v%s keeps the %s line's contract %s: a drop-in client swap."
                  % (version, line, chash))
