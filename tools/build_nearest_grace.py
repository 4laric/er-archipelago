...
# Complete tracked 1.17 output measured 2026-08-30. This table feeds every layer-4 location
# description; a broken coordinate/grace join must not publish a small but plausible answer (#531).
MIN_MATCHED_CHECKS = 3761
...
def require_complete_mapping(mapping, out_path, force=False):
    """Refuse to replace the tracked derivation with a silently shrunken answer."""
    if len(mapping) < MIN_MATCHED_CHECKS and not force:
        raise SystemExit(
            f"FATAL: nearest-grace derivation matched only {len(mapping)} checks "
            f"(floor {MIN_MATCHED_CHECKS}, measured 2026-08-30). Refusing to overwrite {out_path}."
        )
...
def main(argv=None):
    ...
    ap.add_argument("--force", action="store_true",
                    help="allow a deliberately partial/custom input to write fewer than the "
                         f"measured {MIN_MATCHED_CHECKS}-check floor")
    ...
    require_complete_mapping(mapping, args.out, force=args.force)
