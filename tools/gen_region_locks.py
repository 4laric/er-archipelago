...
same two tables every seed's slot_data is built from:
  - greenfield/eldenring/tables/region_open_flags.py  : REGION_OPEN_FLAGS (region -> warp-grace open flag)
  - greenfield/eldenring/tables/region_play_ids.py    : REGION_PLAY_IDS  (region -> play_region ids,
...
def load_open_flags():
    ...
    spec = importlib.util.spec_from_file_location(
        "er_gf_region_open_flags", os.path.join(GF, "tables", "region_open_flags.py"))
...
def load_pending():
    ...
    spec = importlib.util.spec_from_file_location(
        "er_gf_region_play_ids_p", os.path.join(GF, "tables", "region_play_ids.py"))
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
...
def load_play_ids():
    ...
    spec = importlib.util.spec_from_file_location(
        "er_gf_region_play_ids", os.path.join(GF, "tables", "region_play_ids.py"))
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
...
def load_seams(rows):
    """Adjudicated raw-ID exceptions and executable witnesses, never a whole-bucket move."""
    owners = {r: (flag, ids) for r, _lock, flag, ids in rows}
    with open(os.path.join(REPO, "greenfield", "seam_landmarks.tsv"),
              encoding="utf-8", newline="") as handle:
        seams = list(csv.DictReader(handle, delimiter="\t"))
    seen = set()
    for seam in seams:
        region = seam["expected_region"]
        bucket = int(seam["kick_bucket"])
        if region not in owners or bucket not in owners[region][1] or owners[region][0] is None:
            raise ValueError(f"{seam['landmark']}: kick bucket has no matching region lock")
        raw = seam["raw_play_region"]
        if raw:
            if int(raw) < 1_000_000 or int(raw) in seen or not seam["runtime_source"]:
                raise ValueError(f"{seam['landmark']}: invalid/duplicate/unprovenanced raw ID")
            seen.add(int(raw))
            split_bucket, split_region = seam["intentional_split"].split(":", 1)
            neighbor = int(seam["neighbor_raw_play_region"])
            if (int(raw) // 100 != int(split_bucket) or neighbor // 100 != int(split_bucket)
                    or neighbor == int(raw) or int(split_bucket) not in owners[split_region][1]):
                raise ValueError(f"{seam['landmark']}: raw witness does not preserve its neighboring bucket")
    return seams


def render_rs(rows):
    seams = load_seams(rows)
    lines = []
    a = lines.append
    a("// @generated by tools/gen_region_locks.py -- DO NOT EDIT BY HAND.")
    a("// Sources: greenfield/eldenring/tables/region_open_flags.py (REGION_OPEN_FLAGS) +")
    a("//          greenfield/eldenring/tables/region_play_ids.py (REGION_PLAY_IDS) +")
    a("//          greenfield/seam_landmarks.tsv (raw arena exceptions and enforcement witnesses).")
    ...
    a("}")
    a("")
    a("/// Exact raw play-region exceptions adjudicated in greenfield/seam_landmarks.tsv.")
    a("/// Resolve these BEFORE dividing by 100; surrounding buckets retain their original owner.")
    a("pub const RAW_PLAY_REGION_BUCKETS: &[(i32, i32)] = &[")
    for seam in seams:
        if seam["raw_play_region"]:
            a(f"    // {seam['landmark']}: {seam['runtime_source']}")
            a(f"    ({seam['raw_play_region']}, {seam['kick_bucket']}),")
    a("];")
    a("")
    ...
    a("")
    a("    #[test]")
    a("    fn seam_landmarks_enforce_each_side_locked_alone() {")
    a("        use crate::region_lock::{kick_decision, sealed_lock_item};")
    a("        let ranges: Vec<[i32; 3]> = REGION_LOCKS.iter().flat_map(|r| {")
    a("            r.play_regions.iter().filter_map(|&p| r.open_flag.map(|f| [p, p, f as i32]))")
    a("        }).collect();")
    a("        let names = REGION_LOCKS.iter().filter_map(|r| {")
    a("            r.open_flag.map(|f| (r.lock_item.to_string(), f))")
    a("        }).collect();")
    a("        for (pr, lock) in [")
    for seam in seams:
        raw = seam["raw_play_region"] or seam["kick_bucket"]
        a(f'            ({raw}, "{esc(seam["expected_region"])} Lock"),')
        if seam["raw_play_region"]:
            _, neighbor_region = seam["intentional_split"].split(":", 1)
            a(f'            ({seam["neighbor_raw_play_region"]}, "{esc(neighbor_region)} Lock"),')
    a("        ] {")
    a("            let flag = by_lock_item(lock).unwrap().open_flag.unwrap();")
    a('            assert!(!kick_decision(pr, &ranges, 0, &|f| f == flag), "{pr}: owner alone open");')
    a('            assert!(kick_decision(pr, &ranges, 0, &|f| f != flag), "{pr}: owner alone closed");')
    a("            assert_eq!(sealed_lock_item(pr, &ranges, &names), Some(lock));")
    a("            assert_eq!(crate::scaling::region_name_for_bucket(crate::region_lock::play_region_bucket(pr)), Some(by_lock_item(lock).unwrap().region));")
    a("            assert!(!kick_decision(pr, &[], 0, &|_| false));")
    a("        }")
    a("    }")
    a("")
    a("    #[test]")
