#!/usr/bin/env python3
"""Generate eldenring/tables/data.py (regions + flag-keyed locations) for the Greenfield ER apworld.
...
eldenring/tables/*.py data modules (data.py, region_open_flags.py, boss_data.py, region_graces.py,
...
# All generated tables land in the eldenring/tables package (#1464); the loader
# (eldenring/table_loader.py) is what the World reads. Nothing else about generation changed.
...
try:
    _bspec = _ilu.spec_from_file_location("_boss_drops", os.path.join(HERE, "eldenring", "tables", "boss_drops.py"))
...
try:
    _rspec = _ilu.spec_from_file_location("_boss_reward_lots", os.path.join(HERE, "eldenring", "tables", "boss_reward_lots.py"))
...
_ARENA_REGION_CURATED = {
    10000850: "Stormveil",
    # Seethewater River Tibia Mariner. Its reward is anchored to grace 76354 on Mt. Gelmir ground
    # (#1076), but Alaric ruled 2026-09-18 that the boss itself is an Altus boss: it hosts Altus's
    # sweep. The Gelmir grace-cluster CHECKS stay in Mt. Gelmir (#1124); only the host moves.
    1038520800: "Altus",
...
OUT_OPEN = os.path.join(HERE, "eldenring", "tables", "region_open_flags.py")
...
OUT_PIDS = os.path.join(HERE, "eldenring", "tables", "region_play_ids.py")
...
OUT_BOSS = os.path.join(HERE, "eldenring", "tables", "boss_data.py")
...
OUT_GRACES = os.path.join(HERE, "eldenring", "tables", "region_graces.py")
...
OUT_CHECKLOTS = os.path.join(HERE, "eldenring", "tables", "check_lots_data.py")
...
OUT_REPEAT = os.path.join(HERE, "eldenring", "tables", "repeatable_goods.py")
...
# REPEATABLE MINE MATERIALS -- source census from tools/datamine_mine_material_sources.py.
# These are asset-model rewards, not checks: the same unflagged ItemLotParam_map row backs every
# placement of a tier.  Emit only rows that are actually placed, ordinary (non-capstone), repeatable,
# and break-on-pickup.  The feature rerolls slot 1 without touching quantity, flag, or respawn fields.
_MINE_TSV = os.path.join(HERE, "mine_material_sources.tsv")
MINE_MATERIAL_LOTS = set()
if os.path.exists(_MINE_TSV):
    with open(_MINE_TSV, newline="", encoding="utf-8") as _fh:
        _mine_rows = csv.DictReader((line for line in _fh if not line.startswith("#")), delimiter="\t")
        for _row in _mine_rows:
            if (_row.get("capstone") == "no" and _row.get("repeatable") == "yes"
                    and _row.get("break_on_pickup") == "yes"):
                MINE_MATERIAL_LOTS.add(int(_row["item_lot_id"]))
if len(MINE_MATERIAL_LOTS) != 11:
    raise RuntimeError(
        f"mine_material_sources.tsv must identify 11 placed ordinary mine lots; "
        f"found {len(MINE_MATERIAL_LOTS)}")

OUT_MINE = os.path.join(HERE, "eldenring", "tables", "mine_material_data.py")
with open(OUT_MINE, "w", newline="\n", encoding="utf-8") as f:
    f.write('"""AUTO-GENERATED by greenfield/gen_data.py -- DO NOT EDIT.\n')
    f.write('Placed repeatable mine-material ItemLotParam_map rows eligible for per-seed reroll.\n')
    f.write('Capstones and unplaced asset templates are deliberately absent."""\n')
    f.write("MINE_MATERIAL_LOTS = " + repr(tuple(sorted(MINE_MATERIAL_LOTS))) + "\n")
print(f"mine_materials: {len(MINE_MATERIAL_LOTS)} placed repeatable non-capstone lots")

OUT_DROPS = os.path.join(HERE, "eldenring", "tables", "enemy_drops_data.py")
...
OUT_STOCK = os.path.join(HERE, "eldenring", "tables", "shop_stock_data.py")
...
OUT_SHOP = os.path.join(HERE, "eldenring", "tables", "shop_data.py")
...
OUT_MISS = os.path.join(HERE, "eldenring", "tables", "missable_locations.py")
...
OUT_ITEMS = os.path.join(HERE, "eldenring", "tables", "item_ids.py")
...
OUT_TIERS = os.path.join(HERE, "eldenring", "tables", "item_tiers.py")
...
OUT_TAGS = os.path.join(HERE, "eldenring", "tables", "location_tags.py")
...
try:
    ...
    _BOSS_GEO = {}
try:
    _brlspec = _ilu.spec_from_file_location("_brl", os.path.join(HERE, "eldenring", "tables", "boss_reward_lots.py"))
...
OUT_SWEEP = os.path.join(HERE, "eldenring", "tables", "boss_sweeps.py")
...
OUT_SPAWN = os.path.join(HERE, "eldenring", "tables", "spawn_trap_data.py")
...
for _mod in _STAMP_MODULES:
    # The stamp KEYS stay bare module basenames (the client-visible _gen_stamp.json shape is
    # unchanged by #1464); only the path they are read from moved into tables/.
    _mp = os.path.join(_TABLES_DIR, _mod)
