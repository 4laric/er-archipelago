...
def sweep_clause(boss_name, tile=None):
    ...
    *"so idk what boss to kill?"* Under SweepSlot the answer is that a boss hands it over -- and the
    name, which is all the AP hint has to work with, never said so.

    # 🛑 "ALSO GRANTED BY", NEVER "KILL"

    The member is an ordinary pickup and stays a valid route, and 106 of 218 sweep triggers have no
    audited region (#671), so we cannot promise the boss is reachable this seed. This phrasing states
    a fact about the world; an imperative would be a promise we cannot keep.
    ...
    encounters, which is barely better than saying nothing, so `tile` disambiguates when given.
    """
...
def with_sweep(desc, boss_name, tile=None):
    """`desc` with the sweep clause appended, for a member whose trigger we can name.

    Returns `desc` unchanged when there is no clause, and the clause alone when there was no
    description to append to -- a check with no locator is exactly the one that most needs telling
    the player a boss hands it over.
    """
