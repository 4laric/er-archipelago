...
def trusted_aps() -> FrozenSet[int]:
    """Return every host the shipping accuracy bar makes safe: certified, or sweep-backed.

    2026-09-23. The bar is: sweep mode is the default, and a sweep-granted check is a reachable
    check -- kill the owning boss and the check registers whatever the pickup's own placement
    did. A check that is NOT swept depends on our placement being right, and is play at your own
    risk. Required progression may not sit on a play-at-your-own-risk check, so corroboration
    ALONE no longer makes a host: the two-family wiki ledger and the oracle-corroborated table
    both only ADMIT a check into consideration, and sweep membership is what makes it a host.
    The one exception is the certified list: direct in-game adjudications by the maintainer.
    """
    return certified_aps() | sweep_backed_aps()


def _swept_aps() -> FrozenSet[int]:
    try:
        from ..tables.boss_sweeps import DUNGEON_SWEEPS
    except ImportError:
        return frozenset()
    return frozenset(ap for members in DUNGEON_SWEEPS.values() for ap in members)


def _missable_aps() -> FrozenSet[int]:
    try:
...
def _oracle_corroborated_aps() -> FrozenSet[int]:
    try:
        from ..tables.oracle_corroborated_hosts import ORACLE_CORROBORATED_APS
    except ImportError:
        return frozenset()
    return frozenset(ORACLE_CORROBORATED_APS)


def sweep_backed_aps() -> FrozenSet[int]:
    """Return the checks that are corroborated by SOME second source AND granted by a boss sweep
    AND not missable AND not finale.

    Corroboration is either the two-family wiki ledger (``tables/evidence_progression_hosts``) or
    a second, independently curated table of the same game agreeing on the flag with no review
    queue disputing it (``tables/oracle_corroborated_hosts``). Either establishes what the check
    IS; sweep membership is what establishes that it can be REACHED regardless of where we filed
    it, which is the whole safety argument (see ``trusted_aps``). About 3,650 checks against the
    ledger's 1,155, which is what lets ``progression_surface: []`` genuinely scatter. Every OTHER
    bar still applies on top: the finale lifecycle bar, the surface exclusions, the
    region-defaulted set. Fail-closed: a missing sweep table admits nothing.
    """
    generated_trusted, _generated_hold = _generated_sets()
    corroborated = frozenset(generated_trusted or ()) | _oracle_corroborated_aps()
    return (corroborated & _swept_aps()) - _missable_aps() - _always_hold_aps()
...
def hold_aps(world, *, trusted: Optional[Iterable[int]] = None,
    ...
    if trusted is None:
        trusted = trusted_aps() if generated_trusted is not None else None
    if trusted is None:
        return frozenset()  # pre-regen bootstrap only
    universe = frozenset(candidates) if candidates is not None else _all_location_aps()
    # The complement makes newly generated, unaudited checks fail closed -- and, since 2026-09-23,
    # it also holds every wiki-TRUSTED row that no boss sweep grants (see trusted_aps). The named
    # HOLD set is retained as an explicit audit result and catches corrupt tables whose rows
    # escaped `data.py`. A direct adjudication, or a sweep-backed corroborated check,
    # intentionally promotes a row out of the generated HOLD complement.
    effective_hold = generated_hold - certified_aps() - sweep_backed_aps()
