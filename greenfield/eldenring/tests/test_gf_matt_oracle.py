...
class MattOracleLogic(unittest.TestCase):
    ...
    # --- C5. the corroborated-hosts table ---------------------------------
    def test_C5_corroborated_is_joined_minus_disputed_queue_rows(self):
        by_flag = dict(OUR_BY_FLAG)
        region_q = {(90001, 7770001): {"status": "open"},
                    (90002, 7770002): {"status": "confirmed-ours"}}
        missable_q = {(90003, 7770003): {"status": "missable"},
                      (90004, 7770004): {"status": "confirmed-not-missable"}}
        got = self.M.check_corroborated(self.rows, by_flag, region_q, missable_q)
        # 90001 disputed (open), 90003 disputed (missable verdict pending application);
        # 90002/90004 ruled in our favour stay; 90005 has no slot of his and is never in.
        self.assertEqual(got, [7770002, 7770004])
        self.assertEqual(self.M.check_corroborated(self.rows, by_flag, {}, {}),
                         [7770001, 7770002, 7770003, 7770004])

    def test_C5_committed_table_is_live_ap_ids_joined_and_undisputed(self):
        """The shipped table is bare ap-ids of ours, every one a live LOCATIONS row, none of them
        in a disputing row of either committed queue. A regen that renumbers, or a queue refresh
        that disputes a promoted row, must come back through --hosts-table."""
        path = os.path.join(REPO, "greenfield", "eldenring", "tables",
                            "oracle_corroborated_hosts.py")
        spec = importlib.util.spec_from_file_location("_oracle_hosts_under_test", path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        hosts = frozenset(mod.ORACLE_CORROBORATED_APS)
        self.assertGreater(len(hosts), 3000)
        by_flag, _ = self.M.load_ours(REPO)
        live = {ap for entries in by_flag.values() for _r, _n, ap in entries}
        self.assertTrue(hosts <= live, sorted(hosts - live)[:10])
        disputed = set()
        for q in (self.M.read_queue(self.M.DEFAULT_REGION_QUEUE),
                  self.M.read_queue(self.M.DEFAULT_MISSABLE_QUEUE)):
            for (_f, ap), row in q.items():
                if (row.get("status") or "open") in self.M.DISPUTED_STATUSES:
                    disputed.add(ap)
        self.assertTrue(hosts.isdisjoint(disputed), sorted(hosts & disputed)[:10])
        src = open(path, encoding="utf-8").read()
        self.assertIn("AUTO-GENERATED by tools/matt_oracle.py --hosts-table", src)
