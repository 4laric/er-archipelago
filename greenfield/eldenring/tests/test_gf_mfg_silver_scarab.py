...
class SilverScarabRecovery(unittest.TestCase):
    def test_supported_route_and_replacement_share_one_region(self):
        ...
        self.assertIn(30200, assignments(world / 'tables/region_play_ids.py')['REGION_PLAY_IDS'][region])
        ...
        self.assertNotIn(30207900, assignments(world / 'tables/data.py')['NOT_RANDOMIZED'])
        # The other talisman's actual scripted combat gate remains unresolved.
        self.assertIn(1050567820, assignments(world / 'tables/data.py')['NOT_RANDOMIZED'])
