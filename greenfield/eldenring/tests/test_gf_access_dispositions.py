...
class AccessDispositionTests(unittest.TestCase):
    ...
    def test_current_census_is_complete_with_encoded_key_gate_option_rows(self):
        ...
        # -6 #1522 tombstoned phantoms (2026-09-25); +1 furnace sibling #1543; previously -10 #1515/#1518
        self.assertEqual(value["checks_total"], 4926)
        ...
        self.assertEqual(value["by_disposition"]["unresolved"], value["checks_total"] - 6)
        self.assertEqual(value["by_risk"]["critical"]["region_sufficient"], 5)
        self.assertEqual(value["by_risk"]["critical"]["encoded"], 18)
        self.assertEqual(value["by_option_set"]["all"]["region_sufficient"], 4)
        self.assertEqual(
            value["by_option_set"][
        ...
        self.assertEqual(
            value["by_option_set"][
                "item_shuffle=true,jagged_peak=kept,legacy_dungeon_keys=true,vanilla_placement=false"
            ]["encoded"],
            1,
        )
        self.assertEqual(
            value["by_option_set"][
                "not(item_shuffle=true,jagged_peak=kept,legacy_dungeon_keys=true,vanilla_placement=false)"
            ]["unresolved"],
            1,
        )
        ...
        self.assertEqual(value["without_access_claim"], 4896)  # 4902 -> 4896, six #1522 tombstones
    ...
    def test_bootstrap_defaults_every_check_to_unresolved(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(len(rows), 4926)  # -6 #1522 tombstones, 2026-09-25
