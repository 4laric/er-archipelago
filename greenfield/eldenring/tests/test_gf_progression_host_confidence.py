...
class ProgressionHostConfidenceTests(unittest.TestCase):
    ...
    def test_every_check_is_classified_and_missing_evidence_is_hold(self):
        ...
        self.assertEqual(4_926, len(rows))  # 4932 -> 4926, #1522 (2026-09-25): six phantom rows retired via greenfield/tombstones.tsv
        ...
        # 3786 -> 3776 on 2026-09-08 (world#1515/#1518): all ten removed rows were HOLD, so the
        # TRUSTED count above is unchanged and the whole delta lands here.
        # 3777 -> 3771 (2026-09-25, #1522): all six tombstoned phantoms were HOLD; TRUSTED unmoved.
        self.assertEqual(3_771, sum(row["confidence"] == builder.HOLD for row in rows))
