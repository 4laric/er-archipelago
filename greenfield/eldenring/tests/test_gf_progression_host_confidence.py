...
class ProgressionHostConfidenceTests(unittest.TestCase):
    ...
    def test_every_check_is_classified_and_missing_evidence_is_hold(self):
        ...
        self.assertEqual(4_940, len(rows))
        ...
        self.assertEqual(1_153, sum(row["confidence"] == builder.TRUSTED for row in rows))
        self.assertEqual(3_787, sum(row["confidence"] == builder.HOLD for row in rows))
