...
RUN (Windows, from the Archipelago root, with your AP python 3.11+, after build.ps1 -Greenfield):
    ...
    python worlds/eldenring/tests/pool_builder_sweep.py --intensity max --compare
    python worlds/eldenring/tests/pool_builder_sweep.py --num-regions 3 --tag nr3
    python worlds/eldenring/tests/pool_builder_sweep.py --no-dlc --juice-cap 200
...
def _find_ap_root():
    ...
    worlds/eldenring. Checked from this script's dir first (deployed under
    worlds/eldenring/tests/) then from cwd, so the script runs from either place."""
    ...
    for start in seeds:
        ...
        while True:
            ...
            if (os.path.isfile(os.path.join(d, "test", "bases.py"))
                    and os.path.isdir(os.path.join(d, "worlds", "eldenring"))):
                return d
...
if AP_ROOT is None:
    sys.exit("[pool_builder_sweep] could not find an Archipelago root with worlds/eldenring "
             "on disk. Deploy the greenfield world first (build.ps1 -Greenfield) and run this "
             "from the Archipelago root or from worlds/eldenring/tests/.")
os.chdir(AP_ROOT)
