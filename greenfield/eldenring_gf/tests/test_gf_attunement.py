...
becomes K seeded-random start-door graces (K = clamp(ceil(n_graces/8),1,3)).
OFF: no regionAttunement, and the region grace emitter uses its default bundle behavior (each region
lock lights all of the region's graces). Also: the assembled slot_data passes the greenfield contract
both ways.
...
class AttunementOff(WorldTestBase):
    game = GAME
    options = {"num_regions": 0}  # gate off (default); region graces use default bundle behavior
    ...
    def test_bundle_grace_behavior(self):
        sd = self.world.fill_slot_data()
        graces = sd[contract.REGION_GRACES]
        # bundle default: each kept region's lock lights that region's WHOLE grace set.
        self.assertTrue(graces, "bundle mode must emit region graces")
        for region, pool in REGION_GRACE_POINTS.items():
            key = f"{region} Lock"
            if key not in graces:
                continue  # region not kept this seed
            self.assertEqual(set(graces[key]), set(pool),
                             f"{region}: bundle mode lights all of the region's graces")
