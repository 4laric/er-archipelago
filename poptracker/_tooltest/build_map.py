...
Imported by gen_poptracker.py (and runnable standalone). Builds, ONLY when a coordinate
dump is present (a Windows randomizer-gen artifact, see SPEC-poptracker-map.md):

    poptracker/maps/lands_between_map.svg   original parchment map (rounded convex hull of the
                                            real point cloud + grace/location dots + region pins)
    ...
    poptracker/maps/region_pins.json        region -> {map, x, y} pixel pins (for the map variant)

Landmass is a STYLIZED hull (not ER's coastline) drawn from coordinate facts => original art,
no game assets. Authored in gx/gz space, so the transform is exact and pins need no manual
calibration. Pure stdlib (no cairosvg / PIL): emits SVG text only.

The coordinate dump has no map-id column yet, so this filters to the base overworld (tileX<55,
drops DLC/underground + sentinels). DLC/underground maps come with the re-dump (SPEC-poptracker-map).
"""
from __future__ import annotations
import csv, glob, json, math, os, re, statistics
from collections import defaultdict

TILE_MAX = 55          # base overworld only
MAP_NAME = "lands_between"

# overworld majors to LABEL on the SVG (kept readable; pins are emitted for ALL regions w/coords)
...
def find_dump(repo_root):
    """Newest ap_location_coords_*.txt (repo root or SoulsRandomizers/), or None."""
...
def key_to_region(apworld_dir):
    """{location key -> region name} parsed from locations.py `location_tables`."""
...
def build(apworld_dir, pack_root, repo_root):
    """Returns (json_targets, text_targets, stats). Empty targets if no dump (graceful skip)."""
    dump = find_dump(repo_root)
    if not dump:
        return [], [], {"map": "skipped (no ap_location_coords_*.txt)"}
    ...
    # region centroids (median, robust)
    ...
    # geometry (authored in gx/gz => exact transform)
    MARGIN, DRAWW = 70, 900
    ...
    pins_svg, labels_svg, pins_json = "", "", {}
    for reg, c in cent.items():
        px, py = w2px(c["gx"], c["gz"])
        pins_json[reg] = {"map": MAP_NAME, "x": round(px), "y": round(py), "n": c["n"]}
...
<g transform="translate({IMGW/2:.0f},58)" text-anchor="middle">
  ...
  <text y="22" font-size="12" fill="#6f5a33" letter-spacing="1.5">Archipelago Tracker Base &#183; region pins placed at real coordinate centroids</text>
    ...
    stats = {"map": MAP_NAME, "map_points": len(pts),
             ...
             "map_regions_pinned": len(pins_json), "map_labeled": sum(1 for r, _ in LABELS if r in cent)}
...
if __name__ == "__main__":
    ...
    jt, tt, st = build(os.path.join(repo, "Archipelago", "worlds", "eldenring"), pack, repo)
    for path, data in jt:
