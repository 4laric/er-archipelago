...
import build_map  # map layer (M3a); returns empty targets when no coordinate dump is present
...
def generate(apworld_dir):
    ...
    # map layer (M3a): adds maps/* targets ONLY when a coord dump exists; no-op otherwise
    map_json, map_text, map_stats = build_map.build(apworld_dir, PACK_ROOT, REPO_ROOT)
