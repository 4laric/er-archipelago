...
def build(apworld_dir, pack_root, repo_root):
    ...
    if not dump:
        ...
        return [], [], {"map": "skipped (no dump); committed pins=%d" % len(pins)}, pins, base_map_def
...
if __name__ == "__main__":
    ...
    jt, tt, st, pins, mdef = build(os.path.join(repo, "Archipelago", "worlds", "eldenring"), pack, repo)
    for path, data in jt:
    ...
    open(os.path.join(pack, "maps", "maps.json"), "w", encoding="utf-8").write(
        json.dumps([mdef], indent=2, ensure_ascii=False) + "\n")
