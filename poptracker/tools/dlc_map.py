...
import build_map  # shared helpers: load_coords, key_to_region, _hull, _densify, _chaikin
...
# greenfield DLC region name -> short display label (others get an unlabeled pin)
LABELS = [
    ("Gravesite", "Gravesite Plain"), ("Belurat", "Belurat"), ("Ensis", "Castle Ensis"),
    ("Charo's", "Charo's Grave"), ("Scadu Altus", "Scadu Altus"), ("Shadow Keep", "Shadow Keep"),
    ("Scaduview", "Scaduview"), ("Enir Ilim", "Enir Ilim"), ("Cerulean", "Cerulean Coast"),
    ("Stone Coffin", "Stone Coffin Fissure"), ("Jagged Peak", "Jagged Peak"),
    ("Rauh Base", "Rauh Base"), ("Ancient Ruins", "Ancient Ruins of Rauh"), ("Abyssal", "Abyssal Woods"),
]

# m61 (Land of Shadow) tiles land in a private tile band in build_map.load_coords: tileX = XX + 100.
DLC_TILE_LO, DLC_TILE_HI = 100, 999   # [lo, hi): the DLC band, below the interior/mip sentinel (999)


def _is_dlc_band(r):
    try:
        return (abs(float(r["gx"])) < 1e8 and abs(float(r["gz"])) < 1e8
                and DLC_TILE_LO <= r["tileX"] < DLC_TILE_HI)
...
def _dlc_regions_from_locations(apworld_dir):
    """greenfield region_spine.DLC_REGIONS (Land of Shadow region set), read under the reader stubs."""
    import gf_read
    mods = gf_read.load_modules(apworld_dir)
    return set(mods["region_spine"].DLC_REGIONS)
...
def build_dlc(apworld_dir, pack_root, repo_root, dlc_regions=None):
    ...
    rows = build_map.load_coords(repo_root)   # greenfield item_grace_coords.tsv, transform pre-applied
    kr = build_map.key_to_region(apworld_dir)

    def is_dlc_pt(r):
        # m61 Land-of-Shadow tiles are the private DLC band (tileX 100..998); graces included too
        return _is_dlc_band(r)

    pts = [(float(r["gx"]), float(r["gz"]), r["type"]) for r in rows if is_dlc_pt(r)]

    if len(pts) < MIN_POINTS:
        stats = _placeholder(pack_root, len(pts), "m61-band",
                             "too few DLC coordinate points - regenerate greenfield/item_grace_coords.tsv")
...
<g transform="translate({IMGW/2:.0f},48)" text-anchor="middle">
  ...
  <text y="20" font-size="12" fill="#6f5a33" letter-spacing="1.5">Archipelago Tracker DLC &#183; region pins at real coordinate centroids</text>
    ...
    cal = {"note": "GENERATED by tools/dlc_map.py. Authored in gx/gz space; transform is EXACT. +Z is North (up).",
           ...
           "filter": f"DLC overworld via m61 tile band (tileX {DLC_TILE_LO}..{DLC_TILE_HI - 1})"}
    ...
    stats = {"dlc_map": MAP_NAME, "dlc_map_points": len(pts),
             ...
             "dlc_map_filter": "m61-band", "dlc_map_raster": raster}
