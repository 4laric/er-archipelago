...
# greenfield\eldenring_gf. This is the invariant that retires the "NEEDS WINDOWS REGEN" marker: a
...
if (-not $SkipGreenfield) {
    Invoke-CiStep "FRESHNESS (gen-input stamp vs greenfield generated data)" {
        ...
        $stamp    = Join-Path $Repo "greenfield\eldenring_gf\_gen_stamp.json"
        if (-not (Test-Path $manifest)) { throw "FRESHNESS: tools\gen_manifest.py not found -- cannot verify gen-input freshness" }
        if (-not (Test-Path $stamp))    { throw "FRESHNESS: greenfield\eldenring_gf\_gen_stamp.json missing -- data was generated by a pre-stamp gen_data.py. Run: .\build.ps1 -Greenfield" }
